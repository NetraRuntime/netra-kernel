...
//
// Qwen3.6 GDN target-verification fused QKVZ/BA split plus causal
// convolution for gfx950. Exact shape family: B>=1, M=12, QKVZ=12288,
// BA=64, QKV=8192, width=4, BF16, SiLU.
//
// Grid X 0..31 owns 256 QKV features for convolution. Grid X 32..47 owns
// 256 Z features for split materialization; copy block zero also handles BA.
// Grid Y selects the sequence. Independent copy and convolution workgroups
// allow the scheduler to overlap both parts while retaining one graph node.
	...
	global_store_dword v3, v29, s[14:15]
	global_store_short v3, v30, s[14:15] offset:4
	...
	global_store_short v2, v28, s[18:19]
	...
	.macro COPY_Z_TOKEN
	global_load_ushort v38, v34, s[4:5]
	s_waitcnt vmcnt(0)
	global_store_short v35, v38, s[32:33]
	v_add_u32_e32 v34, 0x6000, v34
	v_add_u32_e32 v35, 0x2000, v35
	.endm

	.macro COPY_BA_TOKEN
	global_load_ushort v38, v34, s[6:7]
	global_load_ushort v39, v34, s[6:7] offset:64
	s_waitcnt vmcnt(0)
	global_store_short v35, v38, s[34:35]
	global_store_short v35, v39, s[36:37]
	v_add_u32_e32 v34, 0x80, v34
	v_add_u32_e32 v35, 0x40, v35
	.endm

	.protected qwen36_gdn_fused_split_conv_m12_gfx950
	.globl qwen36_gdn_fused_split_conv_m12_gfx950
	.p2align 8
	.type qwen36_gdn_fused_split_conv_m12_gfx950,@function
qwen36_gdn_fused_split_conv_m12_gfx950:
	// Kernargs: qkvz, ba, weight, state, state_indices, intermediate_window,
	// intermediate_indices, qkv_output, z_output, b_output, a_output, batch.
	s_mov_b32 s20, s2
	...
	s_load_dwordx2 s[32:33], s[0:1], 64
	s_load_dwordx2 s[34:35], s[0:1], 72
	s_load_dwordx2 s[36:37], s[0:1], 80
	s_load_dword s38, s[0:1], 88
	s_waitcnt lgkmcnt(0)

	// Two-dimensional grid: X selects a conv/copy block, Y the sequence.
	s_mov_b32 s21, s3
	s_mov_b32 s22, s20
	s_cmp_ge_u32 s21, s38
	s_cbranch_scc1 .Lend
	s_cmp_ge_u32 s22, 32
	s_cbranch_scc1 .Lcopy_setup
	s_lshl_b32 s25, s21, 2
	s_load_dword s23, s[12:13], s25
	s_load_dword s24, s[16:17], s25
	s_waitcnt lgkmcnt(0)
	...
	// Sequence-local QKVZ source base plus the 256-feature QKV block.
	...
	// Sequence-local packed QKV output base uses [B,12,8192].
	...
	global_load_dword v30, v3, s[10:11]
	global_load_ushort v6, v3, s[10:11] offset:4
	global_load_dwordx2 v[16:17], v2, s[8:9]
	s_waitcnt vmcnt(0)
	...
	// Input and output token offsets differ because QKVZ rows are 12288
	// elements while the packed QKV output rows are 8192 elements.
	...
	v_lshlrev_b32_e32 v3, 2, v0
	v_lshl_add_u32 v3, v0, 1, v3
	global_store_dword v3, v29, s[10:11]
	global_store_short v3, v30, s[10:11] offset:4
	s_branch .Lend

	// Copy workgroups materialize Z within the same dispatch. Re-map 192 lanes to
	// twelve tokens by sixteen 32-byte chunks so each lane issues two x4 loads
	// and two x4 stores instead of twelve scalar pairs.
.Lcopy_setup:
	s_sub_u32 s22, s22, 32
	// Sequence-local QKVZ source and compact Z destination, plus this block.
	s_mul_i32 s25, s21, 0x48000
	s_lshl_b32 s26, s22, 9
	s_add_u32 s25, s25, s26
	s_add_u32 s4, s4, s25
	s_addc_u32 s5, s5, 0
	s_mul_i32 s25, s21, 0x18000
	s_add_u32 s25, s25, s26
	s_add_u32 s32, s32, s25
	s_addc_u32 s33, s33, 0
	v_cmp_gt_u32_e32 vcc, 192, v0
	s_and_saveexec_b64 s[40:41], vcc
	s_cbranch_execz .Lz_done
	v_lshrrev_b32_e32 v36, 4, v0
	v_and_b32_e32 v37, 15, v0
	v_lshlrev_b32_e32 v37, 5, v37
	s_mov_b32 s42, 0x6000
	v_mul_lo_u32 v34, v36, s42
	v_add_u32_e32 v34, 0x4000, v34
	v_add_u32_e32 v34, v37, v34
	v_lshlrev_b32_e32 v35, 13, v36
	v_add_u32_e32 v35, v37, v35
	global_load_dwordx4 v[38:41], v34, s[4:5]
	global_load_dwordx4 v[42:45], v34, s[4:5] offset:16
	s_waitcnt vmcnt(0)
	global_store_dwordx4 v35, v[38:41], s[32:33]
	global_store_dwordx4 v35, v[42:45], s[32:33] offset:16
.Lz_done:
	s_mov_b64 exec, s[40:41]

.Lcopy_ba:
	// Block zero also copies the two 32-wide BA rows. Forty-eight lanes map to
	// twelve tokens by four 16-byte chunks and move B/A together.
	s_cmp_eq_u32 s22, 0
	s_cbranch_scc0 .Lend
	// BA source is [B,12,64], and B/A outputs are [B,12,32].
	s_mul_i32 s25, s21, 0x600
	s_add_u32 s6, s6, s25
	s_addc_u32 s7, s7, 0
	s_mul_i32 s25, s21, 0x300
	...
	v_add_u32_e32 v35, v37, v35
	global_load_dwordx4 v[38:41], v34, s[6:7]
	global_load_dwordx4 v[42:45], v34, s[6:7] offset:64
	s_waitcnt vmcnt(0)
	global_store_dwordx4 v35, v[38:41], s[34:35]
	global_store_dwordx4 v35, v[42:45], s[36:37]
.Lba_done:
	s_mov_b64 exec, s[40:41]
.Lend:
	s_endpgm
	...
	.p2align 6, 0
	.amdhsa_kernel qwen36_gdn_fused_split_conv_m12_gfx950
	...
	.text
.Lfunc_end0:
	.size qwen36_gdn_fused_split_conv_m12_gfx950, .Lfunc_end0-qwen36_gdn_fused_split_conv_m12_gfx950
...
amdhsa.kernels:
  - .args:
      ...
      - { .name: ba_bf16, .offset: 8, .size: 8, .value_kind: global_buffer,
          .address_space: global, .actual_access: read_only }
      - { .name: weight_bf16, .offset: 16, .size: 8, .value_kind: global_buffer,
          .address_space: global, .actual_access: read_only }
      - { .name: state_bf16, .offset: 24, .size: 8, .value_kind: global_buffer,
          .address_space: global, .actual_access: read_write }
      - { .name: state_indices_i32, .offset: 32, .size: 8,
          .value_kind: global_buffer, .address_space: global,
          .actual_access: read_only }
      - { .name: intermediate_window_bf16, .offset: 40, .size: 8,
          .value_kind: global_buffer, .address_space: global,
          .actual_access: write_only }
      - { .name: intermediate_indices_i32, .offset: 48, .size: 8,
          .value_kind: global_buffer, .address_space: global,
          .actual_access: read_only }
      - { .name: qkv_output_bf16, .offset: 56, .size: 8,
          .value_kind: global_buffer, .address_space: global,
          .actual_access: write_only }
      - { .name: z_output_bf16, .offset: 64, .size: 8,
          .value_kind: global_buffer, .address_space: global,
          .actual_access: write_only }
      - { .name: b_output_bf16, .offset: 72, .size: 8,
          .value_kind: global_buffer, .address_space: global,
          .actual_access: write_only }
      - { .name: a_output_bf16, .offset: 80, .size: 8,
          .value_kind: global_buffer, .address_space: global,
          .actual_access: write_only }
      - { .name: batch, .offset: 88, .size: 4, .value_kind: by_value }
    ...
    .name: qwen36_gdn_fused_split_conv_m12_gfx950
    .private_segment_fixed_size: 0
    .sgpr_count: 46
    .sgpr_spill_count: 0
    .symbol: qwen36_gdn_fused_split_conv_m12_gfx950.kd
