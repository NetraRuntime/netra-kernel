...
//
// Rejected Qwen3.6 FP8 E4M3 global-to-LDS transfer experiment.  This keeps
// the retained one-workgroup arithmetic but replaces the W13 row-major
// global-load + ds_write stage with the gfx950 MUBUF LDS opcode observed in
// AITER.  It is intentionally retained as raw assembly because the isolated
// real-checkpoint gate proved that AITER's transfer is coupled to a different
// LDS/MFMA fragment layout; this source is not safe for serving.
//
...
qwen36_moe_fused_m64n256_partial_fp8_gfx950:
	...
	// Cooperative global-to-LDS address.  Loader waves 0 and 1 each own a
	// 1 KiB slice of every 2 KiB row; waves 2 and 3 only consume LDS.
	v_and_b32_e32 v92, 127, v0
	v_lshlrev_b32_e32 v92, 4, v92
	v_lshrrev_b32_e32 v96, 6, v0
	v_readfirstlane_b32 s87, v96
	s_lshl_b32 s87, s87, 10
...
.L_0274:
	...
	// Raw buffer descriptor for the full W13 expert slab.  This matches
	// the descriptor form observed in the deployed AITER gfx950 object.
	s_mov_b32 s88, s36
	s_and_b32 s89, s37, 0xffff
	s_or_b32 s89, s89, 0x40000
	s_mov_b32 s90, 0x120000
	...
	s_mov_b32 s38, 0
	// Prime the current W13 K slab directly into LDS.  Eight cooperative
	// loads replace eight global-load + ds_write pairs.
	s_lshl_b32 s39, s38, 11
	v_add_u32_e32 v95, s39, v92
	s_cmp_lt_u32 s87, 2048
	s_cbranch_scc0 .L_g2lds_w13_prime_done
	s_add_u32 m0, s87, 0x8000
	buffer_load_dwordx4 v95, s[88:91], 0 offen lds
	v_add_u32_e32 v96, 0x8000, v95
	s_add_u32 m0, s87, 0x8800
	buffer_load_dwordx4 v96, s[88:91], 0 offen lds
	v_add_u32_e32 v96, 0x10000, v95
	s_add_u32 m0, s87, 0x9000
	buffer_load_dwordx4 v96, s[88:91], 0 offen lds
	v_add_u32_e32 v96, 0x18000, v95
	s_add_u32 m0, s87, 0x9800
	buffer_load_dwordx4 v96, s[88:91], 0 offen lds
	v_add_u32_e32 v96, 0x100000, v95
	s_add_u32 m0, s87, 0xa000
	buffer_load_dwordx4 v96, s[88:91], 0 offen lds
	v_add_u32_e32 v96, 0x108000, v95
	s_add_u32 m0, s87, 0xa800
	buffer_load_dwordx4 v96, s[88:91], 0 offen lds
	v_add_u32_e32 v96, 0x110000, v95
	s_add_u32 m0, s87, 0xb000
	buffer_load_dwordx4 v96, s[88:91], 0 offen lds
	v_add_u32_e32 v96, 0x118000, v95
	s_add_u32 m0, s87, 0xb800
	buffer_load_dwordx4 v96, s[88:91], 0 offen lds
.L_g2lds_w13_prime_done:
	...
	// Loader and consumer waves have different VMEM counts, so this first
	// correctness-oriented schedule drains both B and A before the barrier.
.L_0358:
...
.L_g2lds_w13_ready:
	...
	s_add_u32 s38, s38, 1
	s_cmp_eq_u64 s[66:67], 0
	s_cbranch_scc1 .L_g2lds_w13_tail
...
.L_g2lds_w13_tail:
	// No wave may overwrite the shared slab until all waves completed both
	// LDS read phases.  This is the cost isolated by this experiment.
	s_barrier
	s_cmp_lt_u32 s38, 16
	...
	v_add_u32_e32 v95, s39, v92
	s_cmp_lt_u32 s87, 2048
	s_cbranch_scc0 .L_g2lds_w13_next_done
	s_add_u32 m0, s87, 0x8000
	buffer_load_dwordx4 v95, s[88:91], 0 offen lds
	v_add_u32_e32 v96, 0x8000, v95
	s_add_u32 m0, s87, 0x8800
	...
	s_add_u32 m0, s87, 0xb800
	buffer_load_dwordx4 v96, s[88:91], 0 offen lds
.L_g2lds_w13_next_done:
	s_cmp_eq_u64 s[66:67], 0
	s_cbranch_scc1 .L_0358
	// Append the six A/scalar loads behind the eight LDS transfers.  The
	// next loop-head vmcnt(6) drains B and leaves A in flight.
	s_lshl_b32 s54, s38, 7
	v_add_u32_e32 v96, s54, v94
	global_load_dwordx4 v[8:11], v96, s[4:5]
	global_load_dwordx4 v[12:15], v96, s[4:5] offset:64
