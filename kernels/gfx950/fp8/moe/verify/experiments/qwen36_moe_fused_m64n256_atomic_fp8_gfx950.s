...
//
// Four cooperating 4-wave64 workgroups consume an expert-sorted M64 route
// block.  Workgroup x owns one activation K128 slice and its matching W2
// split-K contribution across all 2,048 output columns.  They perform the
...
//
// Grid:      (4 activation/W2 K128 slices, ceil(num_valid_ids/64), 1)
...
qwen36_moe_fused_m64n256_atomic_fp8_gfx950:
	...
	// Workgroup x owns exactly one of the four N128 activation blocks.
	s_mov_b32 s80, s2
.Lw13_group:
...
.Lquant_rows:
	...
	// Restore native coordinates for the W2 split-K phase.
	v_and_b32_e32 v1, 15, v0
	v_and_b32_e32 v2, 48, v0
	v_lshlrev_b32_e32 v93, 4, v2
	v_lshlrev_b32_e32 v92, 4, v1
...
.Lactivation_debug_done:
	...
	// Compact-expert W2 bases.  Each x workgroup emits its one K128 partial
	// across N2048; packed BF16 atomics combine four x workgroups and routes.
...
.Ldown_half:
	// Four N16 output tiles (N64) per persistent output step.
	s_lshl_b32 s30, s80, 15
	s_add_u32 s32, s68, s30
	s_addc_u32 s33, s69, 0
...
.Ldown_k:
	// Cooperative W2 N64xK128 slab at LDS [32K,40K).
	s_lshl_b32 s39, s38, 11
...
.Ldown_skip_a:
	...
	ds_write_b64 v100, v[24:25] offset:2048
	ds_write_b64 v100, v[32:33] offset:4096
	ds_write_b64 v100, v[40:41] offset:6144
	s_waitcnt lgkmcnt(0)
	s_barrier
	s_cmp_eq_u64 s[66:67], 0
	...
	v_mfma_f32_16x16x128_f8f6f4 v[60:63], v[8:15], v[40:47], 0
	s_nop 12
	.macro W2_ACC tmp, acc, scale
	v_mul_f32_e32 v\tmp, s72, v\tmp
	v_fmac_f32_e32 v\acc, v\scale, v\tmp
	.endm
	W2_ACC 48,64,108
	W2_ACC 49,65,109
...
.Ldown_skip_compute:
	...
	s_cmp_eq_u64 s[66:67], 0
	s_cbranch_scc1 .Ldown_half_done
