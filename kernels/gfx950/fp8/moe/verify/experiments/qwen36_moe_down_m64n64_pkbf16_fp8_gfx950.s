...
// apply route weights, and accumulate directly into BF16 token output with
// native packed-BF16 atomics.
// Grid: (32 N64 tiles, num_valid_ids/64, 1), workgroup: 256, LDS: 8 KiB.

	.amdgcn_target "amdgcn-amd-amdhsa--gfx950"
	.amdhsa_code_object_version 6
	.text
	.protected qwen36_moe_down_m64n64_pkbf16_fp8_gfx950
	.globl qwen36_moe_down_m64n64_pkbf16_fp8_gfx950
	.p2align 8
	.type qwen36_moe_down_m64n64_pkbf16_fp8_gfx950,@function
qwen36_moe_down_m64n64_pkbf16_fp8_gfx950:
	// route FP8/scales, shuffled W2/scales, sorted IDs/weights/experts,
	// valid count, BF16 token output, rows.
	...
	v_add_u32_e32 v105, 1, v104
	v_add_u32_e32 v106, 2, v104
	v_add_u32_e32 v107, 3, v104
	...
	global_load_dword v80, v3, s[14:15]
	global_load_dword v81, v3, s[14:15] offset:4
	global_load_dword v82, v3, s[14:15] offset:8
	global_load_dword v83, v3, s[14:15] offset:12
	s_waitcnt vmcnt(0)
	...
	.macro DECODE_RESULT packed, token, route, weight
	v_lshrrev_b32_e32 v6, 24, v\packed
	v_and_b32_e32 v\token, 0x00ffffff, v\packed
	v_mul_lo_u32 v\route, 9, v\token
	v_add_u32_e32 v\route, v6, v\route
	v_cmp_gt_u32_e32 vcc, 9, v6
	v_cndmask_b32_e32 v\token, 0, v\token, vcc
	v_cndmask_b32_e32 v\route, 0, v\route, vcc
	v_cndmask_b32_e32 v\weight, 0, v\weight, vcc
	v_cmp_gt_u32_e32 vcc, s22, v\token
	v_cndmask_b32_e32 v\token, 0, v\token, vcc
	v_cndmask_b32_e32 v\route, 0, v\route, vcc
	v_cndmask_b32_e32 v\weight, 0, v\weight, vcc
	.endm
	DECODE_RESULT 84,88,92,80
	DECODE_RESULT 85,89,93,81
	DECODE_RESULT 86,90,94,82
	DECODE_RESULT 87,91,95,83
...
.Lk_loop:
	...
	// Apply router weights once.
	.macro WEIGHT_ROW a0, a1, a2, a3, weight
	v_mul_f32_e32 v\a0, v\weight, v\a0
	v_mul_f32_e32 v\a1, v\weight, v\a1
	v_mul_f32_e32 v\a2, v\weight, v\a2
	v_mul_f32_e32 v\a3, v\weight, v\a3
	.endm
	WEIGHT_ROW 64,68,72,76,80
	WEIGHT_ROW 65,69,73,77,81
	WEIGHT_ROW 66,70,74,78,82
	WEIGHT_ROW 67,71,75,79,83

	// Reuse the 8 KiB LDS weight slab as a row-major M64xN64 BF16 tile.
	// MFMA result rows are wave*16 + lane[5:4]/4 + {0,1,2,3}.
	v_lshrrev_b32_e32 v112, 6, v0
	v_lshlrev_b32_e32 v112, 4, v112
	v_lshrrev_b32_e32 v113, 2, v2
	v_add_u32_e32 v112, v113, v112
	v_lshlrev_b32_e32 v112, 7, v112
	v_lshlrev_b32_e32 v113, 1, v1
	v_add_u32_e32 v112, v113, v112

	.macro LDS_TILE a0, a1, a2, a3, coloff
	v_add_u32_e32 v114, \coloff, v112
	v_cvt_pk_bf16_f32 v115, v\a0, v\a0
	ds_write_b16 v114, v115
	v_cvt_pk_bf16_f32 v115, v\a1, v\a1
	ds_write_b16 v114, v115 offset:128
	v_cvt_pk_bf16_f32 v115, v\a2, v\a2
	ds_write_b16 v114, v115 offset:256
	v_cvt_pk_bf16_f32 v115, v\a3, v\a3
	ds_write_b16 v114, v115 offset:384
	.endm
	LDS_TILE 64,65,66,67,0
	LDS_TILE 68,69,70,71,32
	LDS_TILE 72,73,74,75,64
	LDS_TILE 76,77,78,79,96
	s_waitcnt lgkmcnt(0)
	s_barrier

	// Four threads cover each local row; each owns eight adjacent BF16 pairs.
	v_lshrrev_b32_e32 v116, 2, v0
	v_add_u32_e32 v117, s23, v116
	v_lshlrev_b32_e32 v117, 2, v117
	global_load_dword v117, v117, s[12:13]
	s_waitcnt vmcnt(0)
	v_lshrrev_b32_e32 v118, 24, v117
	v_and_b32_e32 v119, 0x00ffffff, v117
	v_cmp_gt_u32_e32 vcc, 9, v118
	s_and_saveexec_b64 s[46:47], vcc
	v_cmp_gt_u32_e32 vcc, s22, v119
	s_and_b64 exec, exec, vcc
	v_lshlrev_b32_e32 v120, 7, v116
	v_and_b32_e32 v121, 3, v0
	v_lshlrev_b32_e32 v121, 5, v121
	v_add_u32_e32 v120, v121, v120
	ds_read_b128 v[64:67], v120
	ds_read_b128 v[68:71], v120 offset:16
	s_waitcnt lgkmcnt(0)
	// Token BF16 row stride is 4096 bytes. Each N64 group contributes 128
	// bytes; each thread chunk contributes 32 bytes.
	v_lshlrev_b32_e32 v122, 12, v119
	s_lshl_b32 s39, s28, 1
	v_add_u32_e32 v122, s39, v122
	v_add_u32_e32 v122, v121, v122
	global_atomic_pk_add_bf16 v122, v64, s[20:21]
	global_atomic_pk_add_bf16 v122, v65, s[20:21] offset:4
	global_atomic_pk_add_bf16 v122, v66, s[20:21] offset:8
	global_atomic_pk_add_bf16 v122, v67, s[20:21] offset:12
	global_atomic_pk_add_bf16 v122, v68, s[20:21] offset:16
	global_atomic_pk_add_bf16 v122, v69, s[20:21] offset:20
	global_atomic_pk_add_bf16 v122, v70, s[20:21] offset:24
	global_atomic_pk_add_bf16 v122, v71, s[20:21] offset:28
	s_waitcnt vmcnt(0)
...
.Lend:
	...
	.p2align 6, 0
	.amdhsa_kernel qwen36_moe_down_m64n64_pkbf16_fp8_gfx950
	...
	.text
.Lfunc_end0:
	.size qwen36_moe_down_m64n64_pkbf16_fp8_gfx950, .Lfunc_end0-qwen36_moe_down_m64n64_pkbf16_fp8_gfx950
...
amdhsa.kernels:
  - .args:
    ...
    .name: qwen36_moe_down_m64n64_pkbf16_fp8_gfx950
    .private_segment_fixed_size: 0
    .sgpr_count: 50
    .sgpr_spill_count: 0
    .symbol: qwen36_moe_down_m64n64_pkbf16_fp8_gfx950.kd
