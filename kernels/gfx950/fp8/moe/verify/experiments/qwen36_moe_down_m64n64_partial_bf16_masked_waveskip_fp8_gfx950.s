...
// write deterministic BF16 route partials for the fixed-order raw reducer.
...
// Grid: (32 N64 tiles, num_valid_ids/64, 1), workgroup: 256, LDS: 8 KiB.
	...
	.text
	.protected qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950
	.globl qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950
	.p2align 8
	.type qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950,@function
qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950:
	// route FP8/scales, shuffled W2/scales, sorted IDs, experts,
	// valid count, BF16 route-partial workspace, rows.
...
.Lskip_compute:
	...
	// Deterministic unweighted BF16 route workspace. Four result rows and
	// four N16 tiles are contiguous in N for each route.
	// Round each FP32 accumulator once to model-native BF16. The low half of
	// each packed result is stored; the reducer expands it back to FP32 and
	// preserves fixed top-k accumulation order.
	.irp r,64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79
	v_cvt_pk_bf16_f32 v\r, v\r, v\r
	.endr
...
.Lend:
	...
	.p2align 6, 0
	.amdhsa_kernel qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950
	...
	.text
.Lfunc_end0:
	.size qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950, .Lfunc_end0-qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950
...
amdhsa.kernels:
  - .args:
      ...
      - { .name: partial_bf16, .offset: 64, .size: 8, .value_kind: global_buffer, .address_space: global, .actual_access: write_only }
    ...
    .name: qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950
    .private_segment_fixed_size: 0
    .sgpr_count: 58
    .sgpr_spill_count: 0
    .symbol: qwen36_moe_down_m64n64_partial_bf16_masked_waveskip_fp8_gfx950.kd
