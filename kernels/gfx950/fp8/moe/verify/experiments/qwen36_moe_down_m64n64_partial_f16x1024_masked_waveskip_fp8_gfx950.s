...
// write deterministic scaled-FP16 route partials for the fixed-order raw reducer.
// Padded M64 rows are suppressed at the VMEM boundary instead of writing the
// large ignored sink workspace.
// A fully padded M16 wave still cooperates in every W2 load and LDS barrier,
// but skips its activation/scales, LDS reads, MFMAs, and accumulation.
...
// Grid: (32 N64 tiles, num_valid_ids/64, 1), workgroup: 256, LDS: 8 KiB.
	...
	.text
	.protected qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950
	.globl qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950
	.p2align 8
	.type qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950,@function
qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950:
	// route FP8/scales, shuffled W2/scales, sorted IDs, experts,
	// valid count, scaled-FP16 route-partial workspace, rows.
	...
	// Preserve a wave-uniform bitmask indicating whether any of this wave's
	// sixteen sorted rows is real. s[52:55] are otherwise unused until the
	// post-compute store-mask construction.
	v_lshrrev_b32_e32 v6, 24, v4
	v_and_b32_e32 v7, 0x00ffffff, v4
	v_cmp_gt_u32_e64 s[54:55], 9, v6
	v_cmp_gt_u32_e64 s[52:53], s22, v7
...
.Lskip_compute:
	...
	// Deterministic unweighted scaled-FP16 route workspace. Four result rows and
	// four N16 tiles are contiguous in N for each route.
	// Round each FP32 accumulator once to FP16. Captured output range and
	// exact layer tolerances are validated before this variant can be accepted.
	// The exact power-of-two scale prevents FP16 underflow without changing
	// mantissa bits; the reducer applies 2^-10 after FP32 accumulation.
	.irp r,64,65,66,67,68,69,70,71,72,73,74,75,76,77,78,79
	v_mul_f32_e32 v\r, 0x44800000, v\r
...
.Lend:
	...
	.p2align 6, 0
	.amdhsa_kernel qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950
	...
	.text
.Lfunc_end0:
	.size qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950, .Lfunc_end0-qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950
...
amdhsa.kernels:
  - .args:
      ...
      - { .name: partial_f16x1024, .offset: 64, .size: 8, .value_kind: global_buffer, .address_space: global, .actual_access: write_only }
    ...
    .name: qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950
    .private_segment_fixed_size: 0
    .sgpr_count: 58
    .sgpr_spill_count: 0
    .symbol: qwen36_moe_down_m64n64_partial_f16x1024_masked_waveskip_fp8_gfx950.kd
