...
// Wave zero loads each 16x128 activation tile once into LDS; all four waves
// reuse it while independently loading a shuffled N=16 expert-weight tile.
// The four K=128 block-scaled MFMA results are accumulated in FP32 and written
// as unweighted per-route partials. A separate fixed-order raw kernel applies
// the nine router weights and reduces to the final BF16 token output.
//
// Grid: (32 output-tile groups, sorted_token_ids.size()/16 blocks, 1)
// Workgroup: (256, 1, 1), four wave64s. LDS: 2 KiB activation tile.

	.amdgcn_target "amdgcn-amd-amdhsa--gfx950"
	.amdhsa_code_object_version 6
	.text
	.protected qwen36_moe_grouped_down_m16n64_fp8_gfx950
	.globl qwen36_moe_grouped_down_m16n64_fp8_gfx950
	.p2align 8
	.type qwen36_moe_grouped_down_m16n64_fp8_gfx950,@function
qwen36_moe_grouped_down_m16n64_fp8_gfx950:
	// Preserve grid coordinates before loading kernargs. Workitem v0 spans
	// four wave64s; derive a scalar output-tile index per wave.
	s_mov_b32 s40, s2
	s_mov_b32 s41, s3
	v_lshrrev_b32_e32 v46, 6, v0
	v_readfirstlane_b32 s37, v46
	...
	s_lshr_b32 s28, s38, 3
	s_lshl_b32 s28, s28, 4
	s_add_u32 s25, s25, s28
	s_add_u32 s28, s10, s25
	s_addc_u32 s29, s11, 0

	// Base A byte offset. A route stride is 512 bytes. LDS mirrors one
	// 16x128 activation tile using the MFMA lane/subgroup byte layout.
...
.Lkblock:
	...
	// Each K block is one native 16x16x128 E4M3 MFMA per wave. Wave zero
	// loads the activation tile once; all four waves reuse it from LDS.
	s_lshl_b32 s34, s31, 7
	s_cmp_eq_u32 s37, 0
	s_cbranch_scc0 .Lskip_a_load
	...
	ds_write_b128 v46, v[14:17] offset:64
.Lskip_a_load:
	...
	s_barrier
	ds_read_b128 v[10:13], v46
	ds_read_b128 v[14:17], v46 offset:64
	s_waitcnt lgkmcnt(0)
	// All waves must finish their reads before wave zero can overwrite LDS.
	s_barrier
...
.Lend:
	...
	.p2align 6, 0
	.amdhsa_kernel qwen36_moe_grouped_down_m16n64_fp8_gfx950
	...
	.text
.Lfunc_end0:
	.size qwen36_moe_grouped_down_m16n64_fp8_gfx950, .Lfunc_end0-qwen36_moe_grouped_down_m16n64_fp8_gfx950
...
amdhsa.kernels:
  - .args:
    ...
    .name: qwen36_moe_grouped_down_m16n64_fp8_gfx950
    .private_segment_fixed_size: 0
    .sgpr_count: 46
    .sgpr_spill_count: 0
    .symbol: qwen36_moe_grouped_down_m16n64_fp8_gfx950.kd
