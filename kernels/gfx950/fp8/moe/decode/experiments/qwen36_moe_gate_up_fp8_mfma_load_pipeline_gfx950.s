...
//
// gfx950 load-pipelined variant of the Qwen3.6 rowwise routed gate/up
// projection. The arithmetic and FP32 accumulation order are identical to the
// accepted control. After each MFMA, the next K block's scalar/vector loads are
// issued before the required 16-cycle MFMA result hazard wait. This overlaps
// the wait window with the following global-memory transaction without relying
// on an implicit gfx950 dependency interlock.
	...
	.endif
	.text
	.protected qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950
	.globl qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950
	.p2align 8
	.type qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950,@function
qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950:
	...
	// Prime K block zero. s32:s33 hold the current activation/weight scales.
	s_mov_b32 s29, 0
	s_mov_b32 s30, 0
	s_mov_b32 s31, 0
...
.Lkblock:
	...
	// Launch the following K block while the preceding MFMA result is in its
	// required 16-cycle hazard window. The explicit wait remains, but it now
	// overlaps the next VMEM/SMEM transaction instead of preceding it.
	...
	// Consume the preceding result in the same FP32 scale/multiply/add order
	// as the control, then rotate the already-loaded scalar scales.
...
.Llast_block:
	...
	.p2align 6, 0
	.amdhsa_kernel qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950
	...
	.text
.Lfunc_end0:
	.size qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950, .Lfunc_end0-qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950
...
amdhsa.kernels:
  - .args:
    ...
    .name: qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950
    .private_segment_fixed_size: 0
    .sgpr_count: 40
    .sgpr_spill_count: 0
    .symbol: qwen36_moe_gate_up_fp8_mfma_load_pipeline_gfx950.kd
