...
// Fixed B=1, Hq=16, Hkv=2, Dq=Dv=256, BF16, causal, SGLang NHD page size 16.
// Experimental page16: each wave loads four physical page anchors and derives four contiguous rows per anchor; current-chunk path is unchanged. Grid=(M/64,4,1), block=512.
...
.macro LOAD_PREFIX_BATCH16 KOFF PTR
  // The raw page16 specialization is entered only for page-aligned 8K prefix
  // tiers. Each wave loads its first physical slot in four logical 16-token
  // pages, then derives the remaining +4/+8/+12 rows inside each page.
  s_add_u32 s30, s28, s29
  s_add_u32 s30, s30, \KOFF
  s_lshl_b32 s30, s30, 3
  .set PG, 0
  .rept 4
    s_load_b64 s[44+PG*2:45+PG*2], s[16:17], s30 offset:(PG*128)
    .set PG, PG+1
  .endr
  s_waitcnt lgkmcnt(0)
  .set BI, 0
...
.protected extend_attention_wmma_n64_page16_gfx1151
.globl extend_attention_wmma_n64_page16_gfx1151
.p2align 8
.type extend_attention_wmma_n64_page16_gfx1151,@function
extend_attention_wmma_n64_page16_gfx1151:
...
.p2align 6, 0
.amdhsa_kernel extend_attention_wmma_n64_page16_gfx1151
...
.text
.Lfunc_end0:
.size extend_attention_wmma_n64_page16_gfx1151, .Lfunc_end0-extend_attention_wmma_n64_page16_gfx1151
...
amdhsa.kernels:
  - .args:
    ...
    .name: extend_attention_wmma_n64_page16_gfx1151
    .private_segment_fixed_size: 0
    .sgpr_count: 64
    .sgpr_spill_count: 0
    .symbol: extend_attention_wmma_n64_page16_gfx1151.kd
