...
// Tile: M64 x N64 softmax, persistent Q, separate P, two N32 K and V phases. Grid=(M/64,16,1), block=128.
...
.macro LOAD_PREFIX_BATCH8 KOFF PTR
  ...
  s_add_u32 s30, s30, \KOFF
  s_lshl_b32 s30, s30, 3
  s_load_b64 s[44:45], s[16:17], s30 offset:0
  s_load_b64 s[46:47], s[16:17], s30 offset:32
  s_load_b64 s[48:49], s[16:17], s30 offset:64
  s_load_b64 s[50:51], s[16:17], s30 offset:96
  s_load_b64 s[52:53], s[16:17], s30 offset:128
  s_load_b64 s[54:55], s[16:17], s30 offset:160
  s_load_b64 s[56:57], s[16:17], s30 offset:192
  s_load_b64 s[58:59], s[16:17], s30 offset:224
  s_waitcnt lgkmcnt(0)
  .set BI, 0
  .rept 8
...
// Load one 32-token source phase into the reusable 16 KiB LDS stage.
// SOFF selects source rows 0 or 32 while LDS rows remain local 0..31.
// Batch eight page-table/global transfers per N32 source phase while keeping
// score VGPRs v24:31 and v192:215 live across the second K phase.
.macro LOAD_PREFIX_32_PHASE SOFF PTR
  s_add_u32 s30, s28, s29
  s_add_u32 s30, s30, \SOFF
  s_lshl_b32 s30, s30, 3
  s_load_b64 s[44:45], s[16:17], s30 offset:0
  s_load_b64 s[46:47], s[16:17], s30 offset:32
  s_load_b64 s[48:49], s[16:17], s30 offset:64
...
.macro LOAD_CURRENT_32_PHASE SOFF PTR
  .set BI, 0
  .rept 8
    s_add_u32 s38, s28, s29
    s_add_u32 s38, s38, (\SOFF+BI*4)
    s_sub_u32 s38, s38, s19
    s_lshl_b32 s38, s38, 10
    s_add_u32 s38, s38, s25
    s_add_u32 s32, s[\PTR], s38
...
.macro COMPUTE_QK_32 OUT0 OUT1
  ...
  .rept 16
    ...
    s_waitcnt lgkmcnt(0)
    v_wmma_f32_16x16x16_bf16 v[\OUT0:\OUT0+7], v[8:15], v[16:23], v[\OUT0:\OUT0+7]
    v_lshlrev_b32_e32 v234, 9, v3
    v_lshl_add_u32 v234, v4, 4, v234
    v_add_nc_u32_e32 v234, (40960+DK*32), v234
    SWIZZLE_K_LDS_READ
    ds_load_b128 v[16:19], v234
    s_waitcnt lgkmcnt(0)
    ds_swizzle_b32 v20, v16 offset:swizzle(SWAP,16)
    ...
    ds_swizzle_b32 v23, v19 offset:swizzle(SWAP,16)
    s_waitcnt lgkmcnt(0)
    v_wmma_f32_16x16x16_bf16 v[\OUT1:\OUT1+7], v[8:15], v[16:23], v[\OUT1:\OUT1+7]
    .set DK, DK+1
  .endr
.endm

// Consume two P fragments against one local N32 V stage.
.macro COMPUTE_PV_32 PBASE
  .set VT, 0
  .rept 2
    ds_load_b128 v[8:11], v233 offset:(\PBASE*32+VT*32)
    s_waitcnt lgkmcnt(0)
    ds_swizzle_b32 v12, v8 offset:swizzle(SWAP,16)
    ds_swizzle_b32 v13, v9 offset:swizzle(SWAP,16)
    ds_swizzle_b32 v14, v10 offset:swizzle(SWAP,16)
...
.macro RESTORE_P_ADDRS
  v_lshlrev_b32_e32 v231, 11, v2
  v_lshl_add_u32 v232, v4, 7, v231
  v_lshl_add_u32 v232, v3, 1, v232
  v_add_nc_u32_e32 v232, 49152, v232
  v_lshl_add_u32 v233, v3, 7, v231
  v_lshlrev_b32_e32 v229, 4, v1
  v_lshl_add_u32 v233, v4, 4, v233
  v_add_nc_u32_e32 v233, 49152, v233
.endm

.protected extend_attention_wmma_n64_split_kv_gfx1151
.globl extend_attention_wmma_n64_split_kv_gfx1151
.p2align 8
.type extend_attention_wmma_n64_split_kv_gfx1151,@function
extend_attention_wmma_n64_split_kv_gfx1151:
  ...
  // Separate 8 KiB P workspace at 48 KiB: wave*2048 plus fragment layout.
  v_lshlrev_b32_e32 v231, 11, v2
  v_lshl_add_u32 v232, v4, 7, v231
  v_lshl_add_u32 v232, v3, 1, v232
  v_add_nc_u32_e32 v232, 49152, v232
...
.Ltile_loop:
  // Q remains persistent because P has a separate LDS region.
  s_barrier

  // Keep four N64 score fragments live, but stage only N32 K at a time.
  ...
  s_cbranch_scc0 .Lload_k0_current
  LOAD_PREFIX_32_PHASE 0, 12
  s_branch .Lk0_loaded
.Lload_k0_current:
  LOAD_CURRENT_32_PHASE 0, 6
.Lk0_loaded:
  s_waitcnt lgkmcnt(0)
  s_barrier
  COMPUTE_QK_32 24, 192

  s_barrier
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Lload_k1_current
  LOAD_PREFIX_32_PHASE 32, 12
  s_branch .Lk1_loaded
.Lload_k1_current:
  LOAD_CURRENT_32_PHASE 32, 6
.Lk1_loaded:
  s_waitcnt lgkmcnt(0)
  s_barrier
  COMPUTE_QK_32 200, 208
  RESTORE_P_ADDRS
...
.Lmask_done:
  ...
  // Transpose four P fragments into the separate 16x64 BF16 workspace.
  .set PT, 0
  .rept 4
    .set PR, 0
    .rept 8
  ...
  // Reuse the 16 KiB stage for two N32 V phases. P stays separate.
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Lload_v0_current
  LOAD_PREFIX_32_PHASE 0, 14
  s_branch .Lv0_loaded
.Lload_v0_current:
  LOAD_CURRENT_32_PHASE 0, 8
.Lv0_loaded:
  RESTORE_P_ADDRS
  s_waitcnt lgkmcnt(0)
  s_barrier
  ...
  v_lshlrev_b32_e32 v229, 4, v1
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Lload_v1_current
  LOAD_PREFIX_32_PHASE 32, 14
  s_branch .Lv1_loaded
.Lload_v1_current:
  LOAD_CURRENT_32_PHASE 32, 8
.Lv1_loaded:
  RESTORE_P_ADDRS
  s_waitcnt lgkmcnt(0)
  s_barrier
...
.p2align 6, 0
.amdhsa_kernel extend_attention_wmma_n64_split_kv_gfx1151
...
.text
.Lfunc_end0:
.size extend_attention_wmma_n64_split_kv_gfx1151, .Lfunc_end0-extend_attention_wmma_n64_split_kv_gfx1151
...
amdhsa.kernels:
  - .args:
    ...
    .name: extend_attention_wmma_n64_split_kv_gfx1151
    .private_segment_fixed_size: 0
    .sgpr_count: 48
    .sgpr_spill_count: 0
    .symbol: extend_attention_wmma_n64_split_kv_gfx1151.kd
