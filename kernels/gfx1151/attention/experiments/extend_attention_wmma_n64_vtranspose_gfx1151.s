...
// Rejected experiment: transpose V during staging to replace narrow PV loads.
...
// V takes the separate column-major path below.
...
// Scatter one row-major V b128 into a column-major 16-row PV tile.
// Eight BF16 stores replace the later narrow row-strided loads.
.macro TRANSPOSE_V_LDS_WRITE SRC
  // s39 is the production row-major byte offset: row * 512.
  s_lshr_b32 s41, s39, 13
  s_lshl_b32 s41, s41, 13
  s_lshr_b32 s43, s39, 8
  s_and_b32 s43, s43, 30
  s_add_u32 s41, s41, s43
  v_lshlrev_b32_e32 v230, 8, v1
  v_add_nc_u32_e32 v230, s41, v230
  v_add_nc_u32_e32 v230, 32768, v230
  ds_write_b16 v230, v[\SRC+0] offset:0
  v_lshrrev_b32_e32 v247, 16, v[\SRC+0]
  ds_write_b16 v230, v247 offset:32
  ds_write_b16 v230, v[\SRC+1] offset:64
  v_lshrrev_b32_e32 v247, 16, v[\SRC+1]
  ds_write_b16 v230, v247 offset:96
  ds_write_b16 v230, v[\SRC+2] offset:128
  v_lshrrev_b32_e32 v247, 16, v[\SRC+2]
  ds_write_b16 v230, v247 offset:160
  ds_write_b16 v230, v[\SRC+3] offset:192
  v_lshrrev_b32_e32 v247, 16, v[\SRC+3]
  ds_write_b16 v230, v247 offset:224
.endm
...
.macro LOAD_PREFIX_BATCH8 KOFF PTR
  ...
  .rept 8
    ...
    .if \PTR == 14
      .if BI < 4
        TRANSPOSE_V_LDS_WRITE (8+BI*4)
      .else
        TRANSPOSE_V_LDS_WRITE (231+(BI-4)*4)
      .endif
    .else
      SWIZZLE_K_LDS_WRITE \PTR
      .if BI < 4
  ...
  .rept 16
    ...
    .if \PTR == 8
      .if BI < 4
        TRANSPOSE_V_LDS_WRITE (8+BI*4)
      .elseif BI < 8
        TRANSPOSE_V_LDS_WRITE (192+(BI-4)*4)
      .elseif BI < 12
        TRANSPOSE_V_LDS_WRITE (208+(BI-8)*4)
      .else
        TRANSPOSE_V_LDS_WRITE (231+(BI-12)*4)
      .endif
    .else
      SWIZZLE_K_LDS_WRITE \PTR
      .if BI < 4
...
.protected extend_attention_wmma_n64_vtranspose_gfx1151
.globl extend_attention_wmma_n64_vtranspose_gfx1151
.p2align 8
.type extend_attention_wmma_n64_vtranspose_gfx1151,@function
extend_attention_wmma_n64_vtranspose_gfx1151:
...
.Lv_loaded:
  ...
  // Four P[16x16] x V[16x256] fragments use column-major V LDS.
  .set VT, 0
  .rept 4
    ...
    ds_swizzle_b32 v15, v11 offset:swizzle(SWAP,16)
    s_waitcnt lgkmcnt(0)
    v_lshlrev_b32_e32 v234, 5, v3
    v_add_nc_u32_e32 v234, (32768+VT*8192), v234
    v_lshlrev_b32_e32 v235, 4, v4
    v_add_nc_u32_e32 v235, v234, v235
    v_xor_b32_e32 v236, 16, v235
    ds_load_b128 v[16:19], v235 offset:0
    ds_load_b128 v[20:23], v236 offset:0
    s_waitcnt lgkmcnt(0)
    .set OB, 0
    .rept 16
      .if (OB % 2) == 0
        .if OB < 15
          ds_load_b128 v[225:228], v235 offset:((OB+1)*512)
          ds_load_b128 v[229:232], v236 offset:((OB+1)*512)
        .endif
      ...
      .else
        .if OB < 15
          ds_load_b128 v[16:19], v235 offset:((OB+1)*512)
          ds_load_b128 v[20:23], v236 offset:((OB+1)*512)
        .endif
...
.p2align 6, 0
.amdhsa_kernel extend_attention_wmma_n64_vtranspose_gfx1151
...
.text
.Lfunc_end0:
.size extend_attention_wmma_n64_vtranspose_gfx1151, .Lfunc_end0-extend_attention_wmma_n64_vtranspose_gfx1151
...
amdhsa.kernels:
  - .args:
    ...
    .name: extend_attention_wmma_n64_vtranspose_gfx1151
    .private_segment_fixed_size: 0
    .sgpr_count: 48
    .sgpr_spill_count: 0
    .symbol: extend_attention_wmma_n64_vtranspose_gfx1151.kd
