...
// Experimental group2 K/V preload: issue V before the P barrier and next K
// before the tile barrier; publish LDS only after the dependency barrier.
...
.protected extend_attention_wmma_n64_group2_kvpreload_gfx1151
.globl extend_attention_wmma_n64_group2_kvpreload_gfx1151
.p2align 8
.type extend_attention_wmma_n64_group2_kvpreload_gfx1151,@function
extend_attention_wmma_n64_group2_kvpreload_gfx1151:
  ...
  // Cold prefetch for tile zero. Non-loader waves reach the tile barrier while
  // the leader issues VMEM; LDS remains untouched until every wave arrives.
  s_cmp_eq_u32 s43, 0
  s_cbranch_scc0 .Linitial_k_prefetched
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Linitial_k_current
  PREFETCH_PREFIX_BATCH16 12
  s_branch .Linitial_k_prefetched
.Linitial_k_current:
  PREFETCH_CURRENT_BATCH16 6
.Linitial_k_prefetched:

.Ltile_loop:
  // All two heads must finish PV before the leader overwrites shared LDS.
  s_barrier
  // Publish the already-prefetched K tile only after all previous V readers
  // have reached the dependency barrier.
  s_cmp_eq_u32 s43, 0
  s_cbranch_scc0 .Lk_loaded
  STORE_PREFETCHED_BATCH16 12
.Lk_loaded:
  RESTORE_Q_P_ADDRS
  s_waitcnt lgkmcnt(0)
  s_barrier
...
.Lmask_done:
  ...
  // V fetch is independent of the P LDS writes. Issue it before the mandatory
  // P barrier, then publish it only after every QK consumer is finished.
  s_cmp_eq_u32 s43, 0
  s_cbranch_scc0 .Lv_prefetched
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Lprefetch_v_current
  PREFETCH_PREFIX_BATCH16 14
  s_branch .Lv_prefetched
.Lprefetch_v_current:
  PREFETCH_CURRENT_BATCH16 8
.Lv_prefetched:
  s_waitcnt lgkmcnt(0)
  s_barrier

  // Only head-group lane 0 publishes V; both heads reuse it.
  s_cmp_eq_u32 s43, 0
  s_cbranch_scc0 .Lv_loaded
  STORE_PREFETCHED_BATCH16 14
.Lv_loaded:
  RESTORE_Q_P_ADDRS
  s_waitcnt lgkmcnt(0)
  s_barrier
  ...
  // Advance first so the loader can issue next-tile K while the other head
  // drains into the tile-boundary barrier.
  s_add_u32 s28, s28, 64
  s_cmp_lt_u32 s28, s27
  s_cbranch_scc0 .Ltiles_done
  s_cmp_eq_u32 s43, 0
  s_cbranch_scc0 .Ltile_loop
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Lprefetch_k_current
  PREFETCH_PREFIX_BATCH16 12
  s_branch .Ltile_loop
.Lprefetch_k_current:
  PREFETCH_CURRENT_BATCH16 6
...
.p2align 6, 0
.amdhsa_kernel extend_attention_wmma_n64_group2_kvpreload_gfx1151
...
.text
.Lfunc_end0:
.size extend_attention_wmma_n64_group2_kvpreload_gfx1151, .Lfunc_end0-extend_attention_wmma_n64_group2_kvpreload_gfx1151
...
amdhsa.kernels:
  - .args:
    ...
    .name: extend_attention_wmma_n64_group2_kvpreload_gfx1151
    .private_segment_fixed_size: 0
    .sgpr_count: 64
    .sgpr_spill_count: 0
    .symbol: extend_attention_wmma_n64_group2_kvpreload_gfx1151.kd
