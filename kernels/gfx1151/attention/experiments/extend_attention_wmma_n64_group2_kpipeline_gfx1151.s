...
// Experimental group2 K pipeline: fetch next K during current PV WMMA,
// then publish K to LDS after the next mandatory tile barrier.
...
.macro LOAD_PREFIX_BATCH16 KOFF PTR
  ...
  .rept 16
    ...
    .elseif BI < 12
      global_load_b128 v[216+(BI-10)*4:219+(BI-10)*4], v229, s[32:33]
    .else
  ...
  .rept 16
    ...
    .elseif BI < 12
      ds_write_b128 v230, v[216+(BI-10)*4:219+(BI-10)*4]
    .else
...
// Split global fetch from LDS publication so VMEM latency can overlap an
// existing workgroup barrier tail. Prefix and current paths use one common
// register layout; the STORE macro preserves the production LDS layout.
.macro PREFETCH_PREFIX_BATCH16 PTR
  s_add_u32 s30, s28, s29
  s_lshl_b32 s30, s30, 3
  .set BI, 0
  .rept 16
...
.macro PREFETCH_CURRENT_BATCH16 PTR
  .set BI, 0
  .rept 16
    s_add_u32 s38, s28, s29
    s_add_u32 s38, s38, (BI*4)
    s_sub_u32 s38, s38, s19
    s_lshl_b32 s38, s38, 10
    s_add_u32 s38, s38, s25
    s_add_u32 s32, s[\PTR], s38
...
.macro STORE_PREFETCHED_BATCH16 PTR
  s_waitcnt vmcnt(0)
  .set BI, 0
  .rept 16
    s_add_u32 s39, s29, (BI*4)
    s_lshl_b32 s39, s39, 9
    v_add_nc_u32_e32 v230, s39, v229
    v_add_nc_u32_e32 v230, 32768, v230
    SWIZZLE_K_LDS_WRITE \PTR
...
// Next-K pipeline storage avoids live PV operands v8:v23/v225:v232 and
// the persistent output accumulators v64:v191.
.macro PREFETCH_K_PV_PREFIX PTR
  s_add_u32 s30, s28, s29
  s_lshl_b32 s30, s30, 3
  .set BI, 0
  .rept 16
  ...
  .rept 16
    ...
    s_addc_u32 s33, s[\PTR+1], 0
    .if BI < 2
      global_load_b128 v[24+BI*4:27+BI*4], v229, s[32:33]
    .elseif BI < 6
      global_load_b128 v[48+(BI-2)*4:51+(BI-2)*4], v229, s[32:33]
    .elseif BI < 14
      global_load_b128 v[192+(BI-6)*4:195+(BI-6)*4], v229, s[32:33]
    .else
      global_load_b128 v[239+(BI-14)*4:242+(BI-14)*4], v229, s[32:33]
    .endif
    .set BI, BI+1
  .endr
.endm

.macro PREFETCH_K_PV_CURRENT PTR
  .set BI, 0
  .rept 16
    s_add_u32 s38, s28, s29
    s_add_u32 s38, s38, (BI*4)
...
.macro STORE_K_PV_PREFETCH PTR
  ...
  .rept 16
    ...
    SWIZZLE_K_LDS_WRITE \PTR
    .if BI < 2
      ds_write_b128 v230, v[24+BI*4:27+BI*4]
    .elseif BI < 6
      ds_write_b128 v230, v[48+(BI-2)*4:51+(BI-2)*4]
    .elseif BI < 14
      ds_write_b128 v230, v[192+(BI-6)*4:195+(BI-6)*4]
    .else
      ds_write_b128 v230, v[239+(BI-14)*4:242+(BI-14)*4]
    .endif
    .set BI, BI+1
  .endr
.endm
...
.protected extend_attention_wmma_n64_group2_kpipeline_gfx1151
.globl extend_attention_wmma_n64_group2_kpipeline_gfx1151
.p2align 8
.type extend_attention_wmma_n64_group2_kpipeline_gfx1151,@function
extend_attention_wmma_n64_group2_kpipeline_gfx1151:
...
.Ltile_loop:
  ...
  s_barrier
  // Tile zero keeps the production load schedule. Later K tiles were fetched
  // before this barrier and are now safe to publish into the shared LDS half.
  s_cmp_eq_u32 s43, 0
  s_cbranch_scc0 .Lk_loaded
  s_cmp_eq_u32 s28, 0
  s_cbranch_scc0 .Lstore_prefetched_k
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Lload_first_k_current
  LOAD_PREFIX_64 12
  s_branch .Lk_loaded
.Lload_first_k_current:
  LOAD_CURRENT_64 6
  s_branch .Lk_loaded
.Lstore_prefetched_k:
  STORE_K_PV_PREFETCH 12
.Lk_loaded:
  RESTORE_Q_P_ADDRS
  s_waitcnt lgkmcnt(0)
  s_barrier
...
.Lmask_done:
  ...
  // Only head-group lane 0 stages V; both heads reuse it after the barrier.
  s_cmp_eq_u32 s43, 0
...
.Lv_loaded:
  ...
  // Move to the next logical K tile before PV. The loader issues its global K
  // reads into registers disjoint from PV operands, so VMEM runs concurrently
  // with current-tile PV WMMA. LDS publication still waits at the next barrier.
  s_add_u32 s28, s28, 64
  s_cmp_lt_u32 s28, s27
  s_cbranch_scc0 .Lnext_k_pipeline_done
  s_cmp_eq_u32 s43, 0
  s_cbranch_scc0 .Lnext_k_pipeline_done
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Lnext_k_pipeline_current
  PREFETCH_K_PV_PREFIX 12
  s_branch .Lnext_k_pipeline_done
.Lnext_k_pipeline_current:
  PREFETCH_K_PV_CURRENT 6
.Lnext_k_pipeline_done:
...
.p2align 6, 0
.amdhsa_kernel extend_attention_wmma_n64_group2_kpipeline_gfx1151
...
.text
.Lfunc_end0:
.size extend_attention_wmma_n64_group2_kpipeline_gfx1151, .Lfunc_end0-extend_attention_wmma_n64_group2_kpipeline_gfx1151
...
amdhsa.kernels:
  - .args:
    ...
    .name: extend_attention_wmma_n64_group2_kpipeline_gfx1151
    .private_segment_fixed_size: 0
    .sgpr_count: 64
    .sgpr_spill_count: 0
    .symbol: extend_attention_wmma_n64_group2_kpipeline_gfx1151.kd
