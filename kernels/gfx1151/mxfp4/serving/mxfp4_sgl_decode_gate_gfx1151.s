...
//
// Raw AMDGCN MXFP4 decode kernel for gfx1151.
//
// Fixed real Qwen3.6-35B-A3B gate/up shape:
//   8 selected experts, N=512, K=2048, two K splits.
//
// Inputs are load-time repacked (not dequantized):
//   packed: [8, K/2, N] u8, adjacent K nibbles in each byte
//   scales: [8, K/32, N] E8M0 u8
//   a:      [K] bf16
//   y:      [8, N] fp32, zeroed by the caller
//
// Launch: grid=(1,16,1), block=(128,1,1). workgroup_id_y encodes
// expert*2 + K-split.
	...
	.protected mxfp4_decode_gate_gfx1151
	.globl mxfp4_decode_gate_gfx1151
	.p2align 8
	.type mxfp4_decode_gate_gfx1151,@function
mxfp4_decode_gate_gfx1151:
	// Kernargs: packed, scales, activation, output.
	s_clause 0x1
	...
	// packed += resident_expert*2^19 + split*2^18.
	s_lshl_b32 s14, s13, 19
	s_lshl_b32 s15, s12, 18
	s_waitcnt_depctr 0
	...
	// scales += resident_expert*2^15 + split*2^14.
	s_lshl_b32 s14, s13, 15
	s_lshl_b32 s15, s12, 14
	s_waitcnt_depctr 0
	s_add_u32 s14, s14, s15
	s_waitcnt_depctr 0
	s_add_u32 s6, s6, s14
	...
	// activation += split*2048 bytes.
	s_lshl_b32 s14, s12, 11
	s_waitcnt_depctr 0
	s_add_u32 s8, s8, s14
	s_addc_u32 s9, s9, 0

	// output += selected-expert slot*2048 bytes.
	s_lshl_b32 s14, s22, 11
	s_waitcnt_depctr 0
	s_add_u32 s10, s10, s14
	s_addc_u32 s11, s11, 0

	// One lane owns four adjacent output columns.
	v_lshlrev_b32_e32 v1, 2, v0

	// E2M1 magnitude -> BF16 byte lookup tables.
...
.Lmxblock:
	...
	// One scale dword covers this lane's four output columns.
	...
	s_waitcnt vmcnt(0)
	// E8M0 exponent byte becomes the IEEE-f32 exponent field directly.
	...
	// Two K-split workgroups meet here. gfx1151 has native non-returning
	// global f32 atomics; avoid the compiler's four CAS loops.
	v_lshlrev_b32_e32 v2, 4, v0
	global_atomic_add_f32 v2, v8, s[10:11]
	global_atomic_add_f32 v2, v9, s[10:11] offset:4
	global_atomic_add_f32 v2, v10, s[10:11] offset:8
	global_atomic_add_f32 v2, v11, s[10:11] offset:12
	s_endpgm

	.section .rodata,"a",@progbits
	.p2align 6, 0
	.amdhsa_kernel mxfp4_decode_gate_gfx1151
	...
	.text
.Lfunc_end0:
	.size mxfp4_decode_gate_gfx1151, .Lfunc_end0-mxfp4_decode_gate_gfx1151
...
amdhsa.kernels:
  - .args:
    ...
    .name: mxfp4_decode_gate_gfx1151
    .private_segment_fixed_size: 0
    .sgpr_count: 26
    .sgpr_spill_count: 0
    .symbol: mxfp4_decode_gate_gfx1151.kd
