...
//   8 selected experts, N=512, K=2048, one MX block per workgroup.
//
// Inputs are load-time repacked (not dequantized):
//   packed: [8, K/2, N] u8, adjacent K nibbles in each byte
//   scales: retained in the kernarg ABI but unused here
//   a:      [K] bf16
//   y:      [64,8,N] unscaled per-MX-block fp32 partials
//
// Launch: grid=(1,512,1), block=(128,1,1). workgroup_id_y encodes
// selected-slot*64 + MX-block.
	...
	.protected mxfp4_decode_gate_block64_gfx1151
	.globl mxfp4_decode_gate_block64_gfx1151
	.p2align 8
	.type mxfp4_decode_gate_block64_gfx1151,@function
mxfp4_decode_gate_block64_gfx1151:
	...
	// workgroup_id_y = selected-slot*64 + MX-block.
	s_and_b32 s12, s3, 63
	s_lshr_b32 s13, s3, 6
	...
	// packed += resident_expert*2^19 + MX-block*2^13.
	s_lshl_b32 s14, s13, 19
	s_lshl_b32 s15, s12, 13
	s_waitcnt_depctr 0
	...
	// activation += block*64 bytes.
	s_lshl_b32 s14, s12, 6
	s_add_u32 s8, s8, s14
	s_addc_u32 s9, s9, 0

	// output += block*16384 + selected-slot*2048 bytes.
	s_lshl_b32 s14, s12, 14
	s_lshl_b32 s15, s22, 11
	s_add_u32 s14, s14, s15
...
.Lmxblock:
	...
	// Store one unscaled MX-block contribution in [block,slot,N] FP32.
	...
	.p2align 6, 0
	.amdhsa_kernel mxfp4_decode_gate_block64_gfx1151
	...
	.text
.Lfunc_end0:
	.size mxfp4_decode_gate_block64_gfx1151, .Lfunc_end0-mxfp4_decode_gate_block64_gfx1151
...
amdhsa.kernels:
  - .args:
    ...
    .name: mxfp4_decode_gate_block64_gfx1151
    .private_segment_fixed_size: 0
    .sgpr_count: 26
    .sgpr_spill_count: 0
    .symbol: mxfp4_decode_gate_block64_gfx1151.kd
