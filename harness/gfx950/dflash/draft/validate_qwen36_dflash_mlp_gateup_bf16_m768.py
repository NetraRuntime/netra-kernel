#!/usr/bin/env python3
"""Validate/time raw gfx950 Qwen3.6 dFlash M768 MLP gate/up projection."""
...
K = 2048
N = 12288
WEIGHT_KEYS = ("layers.0.mlp.gate_proj.weight", "layers.0.mlp.up_proj.weight")
...
def main() -> None:
    ...
    parser.add_argument(
        "--stem",
        default="qwen36_dflash_mlp_gateup_bf16_m768_m128n128_lds128_gfx950",
    )
    ...
    with safe_open(args.checkpoint, framework="pt", device="cpu") as file:
        weight = (
            torch.cat([file.get_tensor(key) for key in WEIGHT_KEYS], dim=0)
            .cuda()
            .contiguous()
        )
    ...
    bridge = args.build_dir / "libqwen36_dflash_mlp_gateup_bf16_m768_bridge.so"
    hsaco = args.build_dir / f"{args.stem}.hsaco"
    library = ctypes.CDLL(str(bridge.resolve()))
    library.netra_qwen36_dflash_mlp_gateup_bf16_m768_load.argtypes = [ctypes.c_char_p]
    library.netra_qwen36_dflash_mlp_gateup_bf16_m768_load.restype = ctypes.c_int
    library.netra_qwen36_dflash_mlp_gateup_bf16_m768_launch.argtypes = [
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
    ]
    library.netra_qwen36_dflash_mlp_gateup_bf16_m768_launch.restype = ctypes.c_int
    library.netra_qwen36_dflash_mlp_gateup_bf16_m768_last_error.restype = ctypes.c_char_p
    rc = library.netra_qwen36_dflash_mlp_gateup_bf16_m768_load(
        str(hsaco.resolve()).encode()
    )
    if rc:
        raise RuntimeError(
            library.netra_qwen36_dflash_mlp_gateup_bf16_m768_last_error().decode()
        )

    def raw() -> torch.Tensor:
        stream = torch.cuda.current_stream()
        rc = library.netra_qwen36_dflash_mlp_gateup_bf16_m768_launch(
