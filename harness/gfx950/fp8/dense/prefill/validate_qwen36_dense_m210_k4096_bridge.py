#!/usr/bin/env python3
"""Validate the gfx950 dense M=210 K=4096 bridge on captured operands."""
...
def main() -> None:
    ...
    parser.add_argument("--n", type=int, choices=(2048,), default=2048)
    ...
    library.netra_qwen36_dense_m210_k4096_load.argtypes = [ctypes.c_char_p]
    library.netra_qwen36_dense_m210_k4096_load.restype = ctypes.c_int
    library.netra_qwen36_dense_m210_k4096_launch.argtypes = [
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
        ctypes.c_void_p,
    ...
    library.netra_qwen36_dense_m210_k4096_launch.restype = ctypes.c_int
    library.netra_qwen36_dense_m210_k4096_last_error.argtypes = []
    library.netra_qwen36_dense_m210_k4096_last_error.restype = ctypes.c_char_p

    def check(status: int) -> None:
        if status == 0:
            return
        error = library.netra_qwen36_dense_m210_k4096_last_error()
        raise RuntimeError(error.decode() if error else f"status={status}")

    check(
        library.netra_qwen36_dense_m210_k4096_load(
    ...
    def launch() -> None:
        check(
            library.netra_qwen36_dense_m210_k4096_launch(
