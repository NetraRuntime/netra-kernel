...
out_dir=${2:-"${repo_dir}/build/gfx950-qwen36-gdn-fused-split-conv-m12"}
rocm_dir=${ROCM_DIR:-/opt/rocm}
stem=qwen36_gdn_fused_split_conv_m12_gfx950
source_file=${repo_dir}/kernels/gfx950/linear_attention/verify/${stem}.s
bridge=${repo_dir}/runtime/gfx950/linear_attention/verify/qwen36_gdn_fused_split_conv_m12_bridge.hip
header=${repo_dir}/runtime/gfx950/linear_attention/verify/qwen36_gdn_fused_split_conv_m12_bridge.h
...
library=${out_dir}/libqwen36_gdn_fused_split_conv_m12_bridge.so
"${rocm_dir}/bin/hipcc" --offload-arch=gfx950 -O3 -std=c++17 -fPIC \
  -shared "$bridge" -o "$library"
nm -D "$library" |
  grep -q 'netra_qwen36_gdn_fused_split_conv_m12_batch_capacity'

sha256sum "$source_file" "$bridge" "$header" "${out_dir}/${stem}.hsaco" \
  "$library" > "${out_dir}/sha256sums.txt"
printf 'target=gfx950\nwavefront_size=64\nshape=B1+,M12,QKVZ12288,BA64,QKV8192,W4\n' \
  > "${out_dir}/build-variant.txt"
echo "gfx950 Qwen GDN M12 fused split-convolution build complete: ${out_dir}"
