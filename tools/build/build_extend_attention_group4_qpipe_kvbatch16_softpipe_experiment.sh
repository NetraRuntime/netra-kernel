...
out_dir=${1:-"${repo_dir}/build/experiments/attention-group4-qpipe-kvbatch16-softpipe"}
rocm=/opt/rocm-7.2.1
clang=${rocm}/llvm/bin/clang
hipcc=${rocm}/bin/hipcc
stem=extend_attention_wmma_n64_group4_qpipe_kvbatch16_softpipe_gfx1151
...
"${hipcc}" --offload-arch=gfx1151 -O3 -shared -fPIC \
  ...
  -DNETRA_EXTEND_ATTENTION_GRID_Y=4 -DNETRA_EXTEND_ATTENTION_BLOCK_X=512 \
  "${repo_dir}/harness/gfx1151/attention/extend_attention_wmma_launcher.hip" \
  -o "${out_dir}/libextend_attention_group4_qpipe_kvbatch16_softpipe.so"
...
echo "Built gfx1151 group4 qpipe kvbatch16 softpipe attention experiment in ${out_dir}"
