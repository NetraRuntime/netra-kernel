...
out_dir=${1:-"${repo_dir}/build/experiments/attention-vtranspose"}
rocm=/opt/rocm-7.2.1
clang=${rocm}/llvm/bin/clang
hipcc=${rocm}/bin/hipcc
stem=extend_attention_wmma_n64_vtranspose_gfx1151
...
"${hipcc}" --offload-arch=gfx1151 -O3 -shared -fPIC \
  "${repo_dir}/harness/gfx1151/attention/extend_attention_wmma_launcher.hip" -o "${out_dir}/libextend_attention_qpipe8.so"
"${hipcc}" --offload-arch=gfx1151 -O3 -shared -fPIC -DNETRA_EXTEND_ATTENTION_KERNEL=${stem} \
  "${repo_dir}/harness/gfx1151/attention/extend_attention_wmma_launcher.hip" -o "${out_dir}/libextend_attention_vtranspose.so"
...
echo "Built gfx1151 V-transpose attention experiment in ${out_dir}"
