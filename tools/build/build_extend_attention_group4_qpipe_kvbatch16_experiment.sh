...
out_dir=${1:-"${repo_dir}/build/experiments/attention-group4-qpipe-kvbatch16"}
rocm=/opt/rocm-7.2.1
clang=${rocm}/llvm/bin/clang
hipcc=${rocm}/bin/hipcc
stem=extend_attention_wmma_n64_group4_qpipe_kvbatch16_gfx1151
baseline_stem=extend_attention_wmma_n64_group4_qpipe_gfx1151
mkdir -p "${out_dir}"
...
"${hipcc}" --offload-arch=gfx1151 -O3 -shared -fPIC \
  ...
  -o "${out_dir}/libextend_attention_group4_qpipe.so"
"${hipcc}" --offload-arch=gfx1151 -O3 -shared -fPIC \
...
echo "Built gfx1151 group4 qpipe kvbatch16 attention experiment in ${out_dir}"
