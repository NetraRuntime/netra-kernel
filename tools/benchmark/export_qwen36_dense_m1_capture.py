#!/usr/bin/env python3
"""Export a retained Qwen3.6 M=1 dense FP8 operand capture."""
...
PREFIX = "gdn_stage.layer.0.dense_m1_n2048_k4096."
EXPECTED = {
    "input_bf16": ((1, 4096), torch.bfloat16),
    "q_input": ((1, 4096), torch.float8_e4m3fn),
    "x_scale": ((1, 32), torch.float32),
    "weight": ((2048, 4096), torch.float8_e4m3fn),
    "weight_scale": ((16, 32), torch.float32),
    "output_bf16": ((1, 2048), torch.bfloat16),
}
...
def main() -> None:
    ...
    reference = torch.zeros((1, 2048), dtype=torch.float32)
    for k_block in range(32):
        k_start = k_block * 128
        k_end = k_start + 128
