...
def main() -> None:
    ...
    library = ctypes.CDLL(
        str(args.build_dir / "libqwen36_gdn_fused_split_conv_m12_bridge.so")
    )
    library.netra_qwen36_gdn_fused_split_conv_m12_load.argtypes = [ctypes.c_char_p]
    library.netra_qwen36_gdn_fused_split_conv_m12_load.restype = ctypes.c_int
    library.netra_qwen36_gdn_fused_split_conv_m12_launch.argtypes = (
        [ctypes.c_void_p] * 11 + [ctypes.c_uint32, ctypes.c_void_p]
    )
    library.netra_qwen36_gdn_fused_split_conv_m12_launch.restype = ctypes.c_int
    library.netra_qwen36_gdn_fused_split_conv_m12_last_error.restype = ctypes.c_char_p
    hsaco = args.build_dir / "qwen36_gdn_fused_split_conv_m12_gfx950.hsaco"
    status = library.netra_qwen36_gdn_fused_split_conv_m12_load(str(hsaco).encode())
    if status:
        raise RuntimeError(
            library.netra_qwen36_gdn_fused_split_conv_m12_last_error().decode()
        )
    ...
    raw_z = torch.empty((rows, 32, 128), dtype=torch.bfloat16, device=device)
    raw_b = torch.empty((rows, 32), dtype=torch.bfloat16, device=device)
    raw_a = torch.empty_like(raw_b)

    def raw_launch() -> None:
        stream = torch.cuda.current_stream(device)
        status = library.netra_qwen36_gdn_fused_split_conv_m12_launch(
            pointer(qkvz), pointer(ba), pointer(weight), pointer(raw_state),
            pointer(state_indices), pointer(raw_window), pointer(intermediate_indices),
            pointer(raw_qkv), pointer(raw_z), pointer(raw_b), pointer(raw_a),
            CAPTURE_BATCH, ctypes.c_void_p(stream.cuda_stream),
        )
        if status:
            raise RuntimeError(
                library.netra_qwen36_gdn_fused_split_conv_m12_last_error().decode()
    ...
    correctness = {
        ...
        "raw_z": compare(raw_z, expected_z),
        "raw_b": compare(raw_b, expected_b),
        "raw_a": compare(raw_a, expected_a),
