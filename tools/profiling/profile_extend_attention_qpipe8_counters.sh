...
mkdir -p "${out_dir}"
counters=(
  OccupancyPercent MeanOccupancyPerActiveCU Wavefronts SQ_BUSY_CYCLES
  FETCH_SIZE L2CacheHit LDSBankConflict VALUInsts SALUInsts
)
for variant in baseline candidate; do
  binary=${build_dir}/extend_attention_counter_${variant}
  if [[ ${variant} == baseline ]]; then
    hsaco=${baseline_hsaco}
    input_scope="synthetic zeros exact Qwen3.6 T8192 prefix24576 Hq16 Hkv2 D256 serial-Q N64"
  else
...
echo "gfx1151 measured qpipe8 counter reports: ${out_dir}/{baseline,candidate}/summary.json"
