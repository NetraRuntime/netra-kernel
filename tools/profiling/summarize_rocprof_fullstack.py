...
FAMILY_RULES: list[tuple[str, re.Pattern[str]]] = [
    ("netra_mxfp4_moe", re.compile(r"netra_mxfp4.*(gate|up|down|decode|prefill|reduce|silu)", re.I)),
    ...
    ("elementwise_copy", re.compile(r"copy|fill|zero|arange|index|gather|scatter|cat|transpose|reshape|contiguous|memcpy", re.I)),
            ...
            rows.append(
                {
                    ...
                    "grid": row.get("Grid_Size") or row.get("grid_size"),
                    "workgroup": row.get("Workgroup_Size") or row.get("workgroup_size"),
                    "vgpr": row.get("vgpr_count") or row.get("VGPR_Count"),
                    "sgpr": row.get("sgpr_count") or row.get("SGPR_Count"),
                    "lds": row.get("lds_size") or row.get("LDS_Size"),
