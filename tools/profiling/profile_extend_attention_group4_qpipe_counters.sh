...
build_dir=${repo_dir}/build/experiments/attention-group4-qpipe
out_dir=${1:-"${repo_dir}/results/profiles/gfx1151/extend-attention-group4-qpipe-$(date -u +%Y%m%dT%H%M%SZ)"}
repeats=${NETRA_ROCPROF_REPEATS:-3}
baseline_hsaco=${build_dir}/extend_attention_wmma_n64_qpipe8_baseline_gfx1151.hsaco
candidate_hsaco=${build_dir}/extend_attention_wmma_n64_group4_qpipe_gfx1151.hsaco
[[ -x ${build_dir}/extend_attention_counter_candidate && -f ${candidate_hsaco} && -f ${baseline_hsaco} ]] || \
  "${repo_dir}/tools/build/build_extend_attention_group4_qpipe_experiment.sh"
mkdir -p "${out_dir}"
counters=(
...
for variant in baseline candidate; do
  ...
  if [[ ${variant} == baseline ]]; then
    ...
    input_scope="synthetic zeros exact Qwen3.6 T8192 prefix24576 Hq16 Hkv2 D256 group4 qpipe shared-KV"
  fi
...
echo "gfx1151 measured group4 qpipe shared-KV counter reports: ${out_dir}/{baseline,candidate}/summary.json"
