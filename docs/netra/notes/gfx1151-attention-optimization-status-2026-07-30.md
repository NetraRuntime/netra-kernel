...
| 6 | Reuse K/V across eight query heads per KV head | Tried in groups of four and two; group2 accepted | Group4 first delivered 1.2279x isolated over the earlier kernel. A 256-thread group2 schedule then traded doubled K/V staging for shorter workgroup/barrier tails: the exact T8192 four-prefix sum improved another 1.04255x, and four interleaved exact 32K/+1 serving pairs improved a combined 0.8593%. The 8K serving delta was within noise. |
...
| 8 | Pipeline next global K/V tile | Tried, rejected | Current-tile K preload improved the four-prefix HIP-event sum only 1.00369x and exact 32K/+1 serving by a noisy 0.116%. True next-K overlap during current PV regressed the long-prefix sum 4.923%. Both remained bit-exact; production is unchanged. |
...
3. The smaller-workgroup branch is now accepted as group2 for long prefill. Revisit LDS residency (#5) only with a materially smaller accumulator/register footprint; 224 VGPR plus 32 KiB was measured insufficient.
