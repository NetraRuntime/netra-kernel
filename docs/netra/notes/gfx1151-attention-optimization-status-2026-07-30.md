...
| 17 | Fuse Q/K norm + RoPE + KV store | Tried; accepted for eager and full M1 decode graph | Raw gfx1151 fuses model-native Gemma RMSNorm, exact BF16 MRoPE, gate extraction, and page-1 K/V storage. It is bit-exact through M32768 and the full 210/+128 greedy sequence; measured mean kernel speedups are 4.2094x M1, 4.3511x M12, 2.1322x M210, 1.2309x M8192, and 1.2168x M32768. Per-layer stable M1 graph outputs pass three paired exact 210/+128 runs with 1.001763x median output-throughput and 1.005415x host-E2E speedups. A shared cross-layer workspace was rejected after progressive replay slowdown. M2-M16 and piecewise replay remain unvalidated. |
...
1. Extend the accepted full M1 graph integration to M2-M16 verify tiers, then validate piecewise prefill with bounded stable workspaces rather than per-layer 8K residency.
