...
`kernels/gfx1151/attention/experiments/extend_attention_wmma_n64_glc_gfx1151.s` differs only by adding gfx11 `glc` to K/V `global_load_b128` instructions. Q loads retain the default policy. The intent was to bypass the per-workgroup near cache for single-use K/V loads and improve sharing through the device-level cache.
...
Before/after gfx1151 disassemblies and their minimal diff are under `docs/notes/disassembly/extend-attention-glc-gfx1151/`. Reproducible harnesses are `harness/gfx1151/attention/extend_attention_counter_harness.hip`, `tools/profiling/profile_extend_attention_counters.sh`, and `tools/benchmark/benchmark_extend_attention_cache_policy.py`.
