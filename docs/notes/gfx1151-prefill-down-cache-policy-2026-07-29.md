...
`kernels/gfx1151/mxfp4/prefill/experiments/mxfp4_prefill_down_wmma_glc_gfx1151.s` is a raw AMDGCN negative variant that adds gfx11 `glc` only to immutable packed-weight and scale loads. Activation and output memory policies are unchanged.
...
Reproducible artifacts are `harness/gfx1151/mxfp4/prefill/prefill_down_cache_policy_harness.hip`, the retained raw variant, and `results/kernels/gfx1151/down-glc-paired.txt`. Before/after disassemblies are under `docs/notes/disassembly/prefill-down-glc-gfx1151/`.
