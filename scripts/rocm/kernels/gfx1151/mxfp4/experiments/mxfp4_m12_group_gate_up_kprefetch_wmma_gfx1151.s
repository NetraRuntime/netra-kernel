...
.macro PREFETCH_FUSED_HALF AEXTRA GBASE UBASE
// Activation and both weight streams are prefetched into registers that are
// dead after the preceding WMMA. No additional VGPR allocation is required.
...
// Initial block and every following block use the same preloaded entry.
s_mov_b32 s26, 64
global_load_ubyte v14, v10, s[6:7]
global_load_ubyte v15, v10, s[10:11]
PREFETCH_FUSED_HALF 0 v9 v9

.Lmxblock:
ZERO_BLOCK_ACCUMULATORS
COMPUTE_FUSED_HALF
v_mov_b32_e32 v63, v14
v_mov_b32_e32 v81, v15
v_add_nc_u32_e32 v12, 4096, v9
PREFETCH_FUSED_HALF 32 v12 v12
COMPUTE_FUSED_HALF

s_sub_u32 s26, s26, 1
s_waitcnt_depctr 0
s_cmp_eq_u32 s26, 0
s_cbranch_scc1 .Llast_block

// Advance to the next K block, start every next-block load, then hide it
// behind the current block's sixteen scale-accumulate instructions.
