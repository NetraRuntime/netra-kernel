...
// Experimental M64xN32 group4-qpipe-kvbatch8-lowlds: four heads share a 16 KiB K/V tile; four 4 KiB P workspaces keep total LDS at 32 KiB. Grid=(M/64,4,1), block=512.
...
.macro LOAD_PREFIX_BATCH8 KOFF PTR
  s_add_u32 s30, s28, s29
  s_add_u32 s30, s30, \KOFF
  s_lshl_b32 s30, s30, 3
  .set BI, 0
...
.macro LOAD_CURRENT_BATCH8 KOFF PTR
  .set BI, 0
  .rept 8
    s_add_u32 s38, s28, s29
...
.macro LOAD_PREFIX_32 PTR
  LOAD_PREFIX_BATCH8 0, \PTR
.endm

.macro LOAD_CURRENT_32 PTR
  LOAD_CURRENT_BATCH8 0, \PTR
.endm

.macro RESTORE_Q_P_ADDRS
  // Q remains global; compact four per-head 4 KiB P workspaces into LDS[0:16K].
  ...
  v_lshl_add_u32 v8, v2, 10, v8
  v_lshl_add_u32 v13, v4, 6, v8
  v_lshl_add_u32 v13, v3, 1, v13
  v_lshl_add_u32 v10, v3, 6, v8
...
.protected extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151
.globl extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151
.p2align 8
.type extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151,@function
extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151:
...
.Ltile_loop:
  ...
  // K phase: the shared 16 KiB LDS half holds 32x256 BF16 K.
...
.Lk_loaded:
  ...
  .rept 16
    ...
    v_add_nc_u32_e32 v0, (16384+DK*32), v0
    SWIZZLE_K_LDS_READ
    ...
    v_lshlrev_b32_e32 v0, 9, v3
    v_lshl_add_u32 v0, v4, 4, v0
    v_add_nc_u32_e32 v0, (24576+DK*32), v0
    SWIZZLE_K_LDS_READ
  ...
  .endr
  RESTORE_Q_P_ADDRS
  // Prefix tiles are fully valid. Both N32 tiles covering the current M64
  // query block use the uniform causal path.
  s_cmp_lt_u32 s28, s19
  s_cbranch_scc0 .Lcausal_mask
  .set FT, 0
  .rept 2
    .set FR, 0
    .rept 8
...
.Lmask_done:
  ...
  // One online-softmax update for the 32-column tile.
  .set SR, 0
  .rept 8
  ...
  // Transpose two P fragments into this head's 16x32 BF16 workspace.
  .set PT, 0
  .rept 2
    .set PR, 0
    .rept 8
      ...
      ds_write_b16 v13, v12 offset:(PT*32+PR*128)
      .set PR, PR+1
    .endr
    .set PT, PT+1
  .endr
...
.Lv_loaded:
  ...
  .rept 2
    ...
    v_add_nc_u32_e32 v0, (16384+VT*8192), v0
    .set OB, 0
...
.p2align 6, 0
.amdhsa_kernel extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151
...
.text
.Lfunc_end0:
.size extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151, .Lfunc_end0-extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151
...
amdhsa.kernels:
  - .args:
    ...
    .name: extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151
    .private_segment_fixed_size: 0
    .sgpr_count: 64
    .sgpr_spill_count: 0
    .symbol: extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151.kd
