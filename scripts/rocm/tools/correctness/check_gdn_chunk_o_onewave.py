#!/usr/bin/env python3
"""Repeated eager/graph gate for the one-wave raw gfx1151 GDN chunk-o."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--library",
        type=Path,
        default=Path("build/experiments/gdn-chunk-o-onewave/libgdn_chunk_o_onewave.so"),
    )
    parser.add_argument(
        "--hsaco",
        type=Path,
        default=Path("build/experiments/gdn-chunk-o-onewave/gdn_chunk_o_bv32_onewave_gfx1151.hsaco"),
    )
    ...
    candidate_lib.netra_gdn_chunk_o_onewave_init.argtypes = [ctypes.c_char_p]
    candidate_lib.netra_gdn_chunk_o_onewave_init.restype = ctypes.c_int
    candidate_lib.netra_gdn_chunk_o_onewave.argtypes = (
        [ctypes.c_void_p] * 8
        + [ctypes.c_float, ctypes.c_uint, ctypes.c_void_p]
    )
    candidate_lib.netra_gdn_chunk_o_onewave.restype = ctypes.c_int
    status = candidate_lib.netra_gdn_chunk_o_onewave_init(
        str(args.hsaco).encode()
    )
    if status:
        raise RuntimeError(f"one-wave module initialization failed with {status}")
    ...
    def raw() -> None:
        ...
        status = candidate_lib.netra_gdn_chunk_o_onewave(
            *pointers,
        ...
        if status:
            raise RuntimeError(f"one-wave launch failed with status {status}")
    ...
    result = {
        "target": "gfx1151",
        "candidate": "raw_gdn_chunk_o_bv32_onewave_gfx1151",
