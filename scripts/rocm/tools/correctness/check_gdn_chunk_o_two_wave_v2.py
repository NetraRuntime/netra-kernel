#!/usr/bin/env python3
"""Repeated eager/graph gate for the two-wave-v2 raw gfx1151 GDN chunk-o."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--library",
        type=Path,
        default=Path("build/experiments/gdn-chunk-o-two-wave-v2/libgdn_chunk_o_two_wave_v2.so"),
    )
    parser.add_argument(
        "--hsaco",
        type=Path,
        default=Path("build/experiments/gdn-chunk-o-two-wave-v2/gdn_chunk_o_bv32_two_wave_v2_gfx1151.hsaco"),
    )
    ...
    candidate_lib.netra_gdn_chunk_o_two_wave_v2_init.argtypes = [ctypes.c_char_p]
    candidate_lib.netra_gdn_chunk_o_two_wave_v2_init.restype = ctypes.c_int
    candidate_lib.netra_gdn_chunk_o_two_wave_v2.argtypes = (
        [ctypes.c_void_p] * 8
        + [ctypes.c_float, ctypes.c_uint, ctypes.c_void_p]
    )
    candidate_lib.netra_gdn_chunk_o_two_wave_v2.restype = ctypes.c_int
    status = candidate_lib.netra_gdn_chunk_o_two_wave_v2_init(
        str(args.hsaco).encode()
    )
    if status:
        raise RuntimeError(f"two-wave-v2 module initialization failed with {status}")
    ...
    def raw() -> None:
        ...
        status = candidate_lib.netra_gdn_chunk_o_two_wave_v2(
            *pointers,
        ...
        if status:
            raise RuntimeError(f"two-wave-v2 launch failed with status {status}")
    ...
    result = {
        "target": "gfx1151",
        "candidate": "raw_gdn_chunk_o_bv32_two_wave_v2_gfx1151",
