...
out=${1:-"${repo}/results/profiles/gfx1151/attention-page16-$(date -u +%Y%m%dT%H%M%SZ)"}
build=${repo}/build/experiments/attention-page16
profiler=/opt/rocm-7.2.1/bin/rocprofv3
"${repo}/scripts/rocm/tools/build/build_extend_attention_page16_experiment.sh" "${build}" >/dev/null
counters=(SALUInsts FETCH_SIZE L2CacheHit OccupancyPercent)
for variant in baseline candidate; do
  if [[ ${variant} == baseline ]]; then
    hsaco=${build}/extend_attention_wmma_n64_gfx1151.hsaco
  else
    hsaco=${build}/extend_attention_wmma_n64_page16_gfx1151.hsaco
  fi
...
echo "gfx1151 measured N64 page16 counters: ${out}"
