...
out_dir=${1:-"${repo_dir}/results/profiles/gfx1151/bf16-attention-output-counters-$(date -u +%Y%m%dT%H%M%SZ)"}
rocm=/opt/rocm-7.2.1
profiler=${rocm}/bin/rocprofv3
build_dir=${repo_dir}/build/experiments/bf16-attention-output
raw_binary=${build_dir}/raw-counter-driver
rocblas_binary=${build_dir}/rocblas-counter-driver
raw_stem=${NETRA_ATTN_OUTPUT_STEM:-bf16_attn_oproj_decode_wave1_gfx1151}
raw_hsaco=${repo_dir}/build/sglang/${raw_stem}.hsaco
raw_symbol=${NETRA_ATTN_OUTPUT_SYMBOL:-${raw_stem}}
...
"${rocm}/bin/hipcc" -O3 --offload-arch=gfx1151 \
  "${repo_dir}/scripts/rocm/harness/gfx1151/dense/bf16_attention_output_decode_counter_driver.hip" \
  -o "$raw_binary"
"${rocm}/bin/hipcc" -O3 --offload-arch=gfx1151 \
  "${repo_dir}/scripts/rocm/harness/gfx1151/dense/bf16_attention_output_rocblas_counter_driver.hip" \
  -L"${rocm}/lib" -lrocblas -o "$rocblas_binary"

raw_grid=${NETRA_ATTN_OUTPUT_GRID:-256}
raw_block=${NETRA_ATTN_OUTPUT_BLOCK:-256}
{
  echo target=gfx1151
  echo measurement_status=measured
  echo estimated_values=false
  echo shape=M1_N2048_K4096_BF16
...
profile_variant() {
  ...
  for counter in "${counters[@]}"; do
    ...
    echo "gfx1151 measured ${variant} BF16 attention output ${counter}: PASS"
  done
}
...
"$python" "$summarizer" "$out_dir/raw" \
  ...
  --input-scope "exact Qwen3.6 M1 N2048 K4096 BF16 raw ASM on gfx1151" \
...
"$python" "$summarizer" "$out_dir/rocblas" \
  ...
  --input-scope "exact Qwen3.6 M1 N2048 K4096 BF16 rocBLAS on gfx1151" \
  --method "one counter per fresh process; one warmup and one measured dispatch" \
  --out "$out_dir/rocblas-summary.json" > "$out_dir/rocblas-summary.stdout"

echo "gfx1151 measured BF16 attention output counter report: $out_dir"
