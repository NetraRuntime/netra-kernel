...
out=${1:-"${repo}/results/profiles/gfx1151/attention-anchor1-$(date -u +%Y%m%dT%H%M%SZ)"}
build=${repo}/build/experiments/attention-anchor1
profiler=/opt/rocm-7.2.1/bin/rocprofv3
"${repo}/scripts/rocm/tools/build/build_extend_attention_anchor1_experiment.sh" "${build}" >/dev/null
counters=(VALUInsts FETCH_SIZE OccupancyPercent SQ_WAVE_CYCLES)
for variant in baseline candidate; do
  if [[ ${variant} == baseline ]]; then
    hsaco=${build}/extend_attention_wmma_n64_gfx1151.hsaco
  else
    hsaco=${build}/extend_attention_wmma_n64_group4_qpipe_kvbatch16_anchor1_gfx1151.hsaco
  fi
...
echo "gfx1151 measured N64 bounded-anchor negative counters: ${out}"
