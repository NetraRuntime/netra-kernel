...
out=${1:-"${repo}/results/profiles/gfx1151/attention-n32-lowlds-$(date -u +%Y%m%dT%H%M%SZ)"}
build=${repo}/build/experiments/attention-n32-lowlds
profiler=/opt/rocm-7.2.1/bin/rocprofv3
"${repo}/scripts/rocm/tools/build/build_extend_attention_n32_lowlds_experiment.sh" "${build}" >/dev/null
...
for variant in baseline candidate; do
  if [[ ${variant} == baseline ]]; then
    ...
    hsaco=${build}/extend_attention_wmma_n32_group4_qpipe_kvbatch8_lowlds_gfx1151.hsaco
  fi
...
echo "gfx1151 measured M64xN32 32-KiB-LDS counters: ${out}"
