...
out_dir=${1:-"${repo_dir}/results/profiles/gfx1151/bf16-lm-head-counters-$(date -u +%Y%m%dT%H%M%SZ)"}
rocm=/opt/rocm-7.2.1
profiler=${rocm}/bin/rocprofv3
build_dir=${repo_dir}/build/experiments/bf16-lm-head
raw_binary=${build_dir}/raw-counter-driver
rocblas_binary=${build_dir}/rocblas-counter-driver
raw_hsaco=${repo_dir}/build/sglang/bf16_lm_head_decode_wave4_gfx1151.hsaco
...
"${rocm}/bin/hipcc" -O3 --offload-arch=gfx1151 \
  "${repo_dir}/scripts/rocm/harness/gfx1151/dense/bf16_lm_head_decode_counter_driver.hip" \
  -o "$raw_binary"
"${rocm}/bin/hipcc" -O3 --offload-arch=gfx1151 \
  "${repo_dir}/scripts/rocm/harness/gfx1151/dense/bf16_lm_head_rocblas_counter_driver.hip" \
...

{
  ...
  echo shape=M1_N248320_K2048_BF16
...
profile_variant() {
  ...
  for counter in "${counters[@]}"; do
    ...
    echo "gfx1151 measured ${variant} BF16 LM-head ${counter}: PASS"
  done
}
...
"$python" "$summarizer" "$out_dir/raw" \
  --kernel-prefix bf16_lm_head_decode_wave4_gfx1151 \
  --input-scope "exact Qwen3.6 M1 N248320 K2048 BF16 raw ASM on gfx1151" \
...
"$python" "$summarizer" "$out_dir/rocblas" \
  ...
  --input-scope "exact Qwen3.6 M1 N248320 K2048 BF16 rocBLAS on gfx1151" \
  --method "one counter per fresh process; one warmup and one measured dispatch" \
  --out "$out_dir/rocblas-summary.json" > "$out_dir/rocblas-summary.stdout"

echo "gfx1151 measured BF16 LM-head counter report: $out_dir"
