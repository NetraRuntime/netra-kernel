...
out=${1:-"${repo}/results/profiles/gfx1151/attention-overlay32k-$(date -u +%Y%m%dT%H%M%SZ)"}
build=${repo}/build/experiments/attention-overlay32k
profiler=/opt/rocm-7.2.1/bin/rocprofv3
"${repo}/scripts/rocm/tools/build/build_extend_attention_overlay32k_experiment.sh" "${build}" >/dev/null
counters=(OccupancyPercent MeanOccupancyPerActiveCU SQ_WAVES FETCH_SIZE WRITE_SIZE L2CacheHit MemUnitBusy LDSBankConflict VALUInsts SQ_INSTS_LDS SQ_INSTS_SALU)
for variant in baseline candidate; do
  if [[ ${variant} == baseline ]]; then
    hsaco=${build}/extend_attention_wmma_n64_gfx1151.hsaco
  else
    hsaco=${build}/extend_attention_wmma_n64_group4_qpipe_kvbatch4_overlay32k_gfx1151.hsaco
  fi
...
echo "gfx1151 measured N64 32-KiB P/V-overlay counters: ${out}"
