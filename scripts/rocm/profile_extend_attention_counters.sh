...
env -u LD_LIBRARY_PATH "${rocm}/bin/hipcc" --offload-arch=gfx1151 -O3 \
  "${repo_dir}/scripts/rocm/extend_attention_wmma_launcher.hip" \
  "${repo_dir}/scripts/rocm/extend_attention_counter_harness.hip" -o "${binary}"
counters=(
...
/root/sglvenv1151/bin/python "${repo_dir}/scripts/rocm/summarize_rocprof_counters.py" \
