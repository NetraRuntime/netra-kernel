#!/usr/bin/env python3
"""Validate and HIP-event time the fixed gfx1151 BF16 attention-output decode kernel."""
...
from netra_gfx1151_sglang import netra_bf16_attention_output_with_output  # noqa: E402
...
def load_real_weights(checkpoint: Path) -> list[torch.Tensor]:
    ...
    for layer, shard in LAYERS_AND_SHARDS:
        ...
        with safe_open(shard_path, framework="pt", device="cpu") as handle:
            weight = handle.get_tensor(prefix + "o_proj.weight").cuda()
        if weight.dtype != torch.bfloat16 or tuple(weight.shape) != (2048, 4096):
            raise RuntimeError(
                f"unexpected layer {layer} attention-output tensor: {weight.dtype} {weight.shape}"
            )
...
def main() -> None:
    ...
    for index in (0, 6, 9):
        ...
        netra_bf16_attention_output_with_output(weights[index], activations[index], outputs[index])
        torch.cuda.synchronize()
    ...
    result = {
        ...
        "dtype": "BF16",
        "weight_working_set_bytes": 10 * 2048 * 4096 * 2,
