...
.macro LOAD_PREFIX_ONE KOFF PTR
  ...
  global_load_b128 v[225:228], v229, s[32:33] glc
  s_waitcnt vmcnt(0)
...
.macro LOAD_PREFIX_BATCH8 KOFF PTR
  ...
  .rept 8
    ...
    s_addc_u32 s33, s[\PTR+1], 0
    .if BI < 4
      global_load_b128 v[8+BI*4:11+BI*4], v229, s[32:33] glc
    .else
      global_load_b128 v[231+(BI-4)*4:234+(BI-4)*4], v229, s[32:33] glc
    .endif
    .set BI, BI+1
  .endr
  s_waitcnt vmcnt(0)
...
.macro LOAD_CURRENT_BATCH16 KOFF PTR
  ...
  .rept 16
    ...
    .if BI < 4
      global_load_b128 v[8+BI*4:11+BI*4], v229, s[32:33] glc
    .elseif BI < 8
      global_load_b128 v[192+(BI-4)*4:195+(BI-4)*4], v229, s[32:33] glc
    .elseif BI < 12
      global_load_b128 v[208+(BI-8)*4:211+(BI-8)*4], v229, s[32:33] glc
    .else
      global_load_b128 v[231+(BI-12)*4:234+(BI-12)*4], v229, s[32:33] glc
    .endif
    .set BI, BI+1
  .endr
  s_waitcnt vmcnt(0)
